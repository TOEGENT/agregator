...
with open(Path(__file__).parents[1] / "catalogs.json", encoding="utf-8") as file:
    catalog_urls = [item["url"] for item in json.load(file)["makita"]]

db = {}

for catalog_number, url in enumerate(catalog_urls, start=1):
    db[url] = {}
    page = 1

    while True:
        responce = requests.get(
            url + f"?p={page}", headers={"User-Agent": "Mozilla/5.0"}
        )
        ...
        for item_number, item in enumerate(items, start=1):
            ...
            for image in picture_tag_elems:
                ...
                image_url = (
                    "https://makita-russia.shop"
                    + source_tag_elem[0].get("data-srcset")
                )
            ...
            print(
                ...
                f"[{bar}] {item_number}/{len(items)} всего: {len(db[url])}",
                end="",
                flush=True,
            )
